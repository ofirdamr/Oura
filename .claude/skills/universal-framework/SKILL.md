...
**Four recovery scenarios live in `references/escalation-and-handoff.md`, not
here — read that file the moment one of them actually happens, don't pre-load
it into a normal task:**
1. Resuming after a usage-limit interruption (don't silently downgrade the
   orchestration mode the Economist picked).
2. PM escalation when a subagent fails or gets stuck (step in personally on
   the top model; don't just retry the same cheap tier).
3. Stop-and-re-consult when this session itself is the one struggling
   (repeating mistakes, several corrected attempts at the same bug).
4. The full session handoff protocol (recognize the trigger, merge before
   recommending, resolve or record every open question, hand over a
...
- **Tooling Scout** — actively looks for a better way to do the work, not just the
  default one, and uses the concrete tool for each lane rather than guessing from
  memory: a connector via `ListConnectors`/`SearchMcpRegistry`/`SuggestConnectors`;
  an existing skill the user already has enabled via `SearchSkills` (falls back to
  `SuggestSkills` if nothing enabled matches); an org plugin via `SearchPlugins`
  (`SuggestPluginInstall` to offer adding it). Only search the lane that's actually
  relevant to the task at hand — don't run all three for a task that's obviously
  none of them. Fits *this specific project's* domain (its stack, its locale, its
  industry) better than solving it from scratch. Flags the find to the PM with the
  concrete task it helps and what adopting it costs (a connection step, a new
  dependency) — the PM decides whether to adopt, this role doesn't unilaterally
  install anything. This runs whenever a new phase or a recurring pain point shows
  up, not as a one-off at project start.
...
**Invoke the `/verify` skill as the mechanical baseline for any nontrivial code
change** — it exercises the change end-to-end rather than trusting a build/
typecheck pass, and bootstraps a project-specific verify routine if this repo
doesn't have one yet. Don't re-derive "did I actually run this" from scratch
every time; that's what the skill is for. What follows below is this project's
*additional* bar for anything visual/UI-facing, which `/verify` alone doesn't
cover — it's on top of the mechanical baseline, not a replacement for it.
...
to ask "are we done" before offering this. The full trigger-recognition,
merge-before-recommending, and open-question protocol is in
`references/escalation-and-handoff.md` (§4 of that file) — read it the moment
a handoff point is actually reached, then emit:
