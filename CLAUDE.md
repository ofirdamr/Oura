...
- **Every UI change must be responsive on BOTH mobile and desktop.** Test and screenshot both viewports before calling anything done. Use `node scripts/qa-shot.mjs <url> <out.png> mobile` and `desktop` — one of each, every time.
