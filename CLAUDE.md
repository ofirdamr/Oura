...
- **When the founder must do a manual step, give dead-simple numbered directions assuming zero technical knowledge.** He is not a programmer. Every step is one plain action with the exact place to do it as a clickable link (Supabase SQL page, GitHub file to copy, etc.). Say what to click and what to paste, in order — never "apply the migration" or "deploy the API" as if he knows how. Prefer a copy-this → paste-here → click-this-button shape. No jargon, no file-path-only instructions, no assumed tools.
