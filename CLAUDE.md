...
- **White-label branding: guest photos carry the PHOTOGRAPHER's brand, never Oura's.** The guest gallery/Photo Editor is white-label — each baked/downloaded photo shows only the photographer studio's prominent name+logo (e.g. "PHOTO SANTOS"), never a "מופעל על ידי Oura"/"powered by Oura" line (`design/screens/oura_final_production_photo_editor_desktop/screen.png` is the authority). Oura's own logo belongs only in app chrome (headers/nav), never composited onto a guest's photo. Corollary: the Photo Editor's frame toggle is the branding switch — frame-off produces a fully clean photo with zero baked branding (no scrim, credit, or title); frame-on bakes the studio brand. Keep `lib/watermark.ts` (export), `components/guest/BrandedFrame.tsx` (viewer), and the editor preview WYSIWYG-identical.
