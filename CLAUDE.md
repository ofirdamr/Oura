...
- **No proceeding past a confirmed defect.** Once an audit/session confirms a real mismatch, bug, or dead link, fix it before starting the next screen/mission — do not catalog it and move on. The only exception is a defect that's explicitly out of scope for this pass (e.g. belongs to a later PRD phase); state that explicitly to the founder instead of silently deferring it.
