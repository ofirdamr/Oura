...
## 📌 OPEN NOW — /admin/branding fidelity audit: BLOCKED on founder mapping (2026-07-13)
Branch `claude/branding-fidelity-audit-mei1jn` (reset onto latest main; PR #33 already merged). Opened all 6 `design/screens/oura_final_production_branding_settings_{desktop,mobile}_{1,2,3}/screen.png`. **What each file actually is (verified by opening the PNGs, NOT the folder names):**
- **desktop_1/2/3 = three iterations of the SAME /admin/branding screen.** The live page (`app/admin/branding/page.tsx`, 663 lines) currently matches **v1/v2** wording: `מיתוג ולוגו:`, `צבעי המותג`, `סימן מים אוטומטי`, `כסף קלאסי`. **desktop_3 is the most evolved** — renamed sections (`ניהול מיתוג`, `זהות המותג`, `סימן מים חכם (Watermark)`), WEBP support, `כסף פלטינום` (not קלאסי), a preview subtitle `כך תיראה הגלריה שלך בזמן אמת`, warmer rust nav accent + nav labels `ניהול לקוחות`/`הגדרות מיתוג`.
- **mobile_2/3 = a SIMPLER, DISTINCT mobile branding design** (logo + studio-name field + one brand color `#E2725B` + save/cancel + bottom nav), NOT a responsive shrink of the desktop panel. **mobile_3** is the Hebrew-localized final (`פוטו סנטוס`); mobile_2 is the Latin-name version.
- **mobile_1 = MISFILED** (CLAUDE.md folder-name trap): its screen.png is the **guest personal gallery** (`התמונות של יונתן לוי`, match-% badges, download-all/share), NOT branding at all.
- **OPEN QUESTION FOR FOUNDER (blocks all wiring — the audit rule is "confirm mapping before wiring"):** (Q1) Is **desktop_3 canonical** (update live page to v3 wording/WEBP/כסף פלטינום/subtitle/rust accent, then fix RTL/drift against it) OR keep v1/v2 (what's live) and fix only RTL/drift/dead-buttons? (Q2) Mobile: **build the dedicated mobile_3 layout** below `md` (like create-event's bottom-sheet in PR #33) OR desktop-responsive-only this pass? No code changed yet — nothing to wire until Q1/Q2 answered.

## 📌 RESOLVED 2026-07-13 — founder's "registered, no auth email" report
NOT a mail-delivery bug. Live GoTrue state (checked via service-role admin API, base `https://voxxhvywzaizyputjqkm.supabase.co`, URL env has a `/rest/v1/` suffix + leading-space quirk — strip both): exactly **1 user total**, `ofirdamr@gmail.com`, confirmed since 2026-07-04; **no new signup landed**. `mailer_autoconfirm:false`. Cause: Supabase suppresses the confirmation email when you "register" an already-confirmed address (anti-enumeration) — UI says "sent," nothing is sent. Fix given to founder: log in at `/login` (not register); `/forgot-password` if needed (but that reset mail also rides Supabase's default sender — the still-open custom-SMTP gap below). Offered to set his password directly via admin API if reset mail doesn't land. No custom SMTP/Resend env is configured — confirms default sender in use.
