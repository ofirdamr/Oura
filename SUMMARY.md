...
**What went live anyway this session** (`oura-web` `5db18c9a`, on PR #28 branch): dropped the "מופעל על ידי Oura" line, frame-off now exports a fully clean photo, enlarged the PHOTO SANTOS studio name/logo — in `lib/watermark.ts`, `components/guest/BrandedFrame.tsx`, `app/photo-editor/page.tsx`, plus the white-label principle added to `CLAUDE.md`. The watermark/BrandedFrame parts also affect the REAL guest-gallery download/share path and DO match the Stitch white-label branding; only the `/photo-editor` screen itself is off-spec.
**⛔ OPEN QUESTION for the founder (do not start new editor work before this is answered):** revert this session's changes entirely, keep only the legitimately-correct gallery-branding parts (Oura-line removal / clean frame-off / prominent studio name on the real `/gallery` download/share), or revert PR #28's freehanded editor altogether? And confirm the photo editor stays parked until its stage.
**Open PRs (named per standing rule):** #28 (this — freehanded editor, off-spec, DO NOT merge pending the decision above), #29 (design-index root-cause docs), #13, plus the stale-diff trio #16/#4/#7 below.
