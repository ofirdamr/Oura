...
**VERIFY NEXT (needs founder + a day):** (1) founder retries selfie at 0.35 — does he match now? (2) confirm after the next 3am cron that `embed-status` still shows 262 embeddings (proves the delete bug is gone). **HARDENING TODO:** add a DB-level guard so `face_embeddings` can never be per-guest-deleted again (migration: block deletes except photo/event cascade, or move guest↔cluster link to its own table) — can't run migrations from sandbox (DB unreachable), needs doing where Supabase is reachable.
