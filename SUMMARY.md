...
### Spec 1: 5-Tier Asset Pipeline
Each photo gets 5 R2 variants generated by `sharp` in the CF queue consumer:
- `original` — JPEG, 10-25MB, max res (no resize). Locked behind explicit print/album flow.
- `desktop` — WebP/AVIF, 2560px wide, 1.5-2.5MB
- `mobile` — WebP/AVIF, 1200px wide, 300-500KB
- `share` — JPEG, 1080px wide, 150-250KB (used as og:image)
- `thumb` — WebP, 400px wide, 30-50KB (masonry grid)

DB change: add `storage_keys JSONB` column to `photos` table (keys: original/desktop/mobile/share/thumb).
Serving: User-Agent detection → mobile gets `mobile` tier, desktop gets `desktop` tier. og:image = `share`.
High-Res Original: presigned R2 URL, only for print/album/explicit download flow.

### Spec 2: Aspect Ratio & Smart Framing Engine
- **Smart Focal-Point Crop**: use `smartcrop.js` or face-detection to center crops (not blind center-crop) when generating 1:1 and 9:16 tiers.
- **Dynamic Canvas Wrapper (9:16)**: horizontal photos wrapped in vertical 9:16 canvas with blurred ambient background (like video platforms), watermark/logo at bottom — no overlap on core image.
- **Social Export Presets** (generated at share-time, not at upload):
  - Feed: 1:1 square or 4:5
  - Story/Reels/TikTok: 9:16 vertical card with branding
  - Original: native 3:2/16:9, no crop
- **UI**: guest sees format picker before share/save ("הורדה לסטורי" / "הורדה לפיד" / "הורדה מקורית")
