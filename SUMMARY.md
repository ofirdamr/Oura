...
- **Cloud Run deploy STILL BLOCKED — GCP IAM permission not applied correctly** — Deploy triggered (run #29869592144, 2026-07-21) but failed with same error: `PERMISSION_DENIED: iam.serviceaccounts.actAs denied on 932309994000-compute@developer.gserviceaccount.com`. The IAM grant was NOT saved to the right place. To fix: go to [GCP IAM → Service Accounts](https://console.cloud.google.com/iam-admin/serviceaccounts?project=ouraforphotographers) → click `932309994000-compute@developer.gserviceaccount.com` → Permissions tab → Grant Access → new principal: `oura-deploy@ouraforphotographers.iam.gserviceaccount.com` → role: **Service Account User** → Save. Then tell next session to re-trigger the Cloud Run deploy workflow (`deploy-cloud-run.yml` on `main` branch, `workflow_dispatch`).
