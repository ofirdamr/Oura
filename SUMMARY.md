...
## 2026-07-11 — Photo Editor white-label branding (PR #28, open draft — the 4 fixes are DONE + LIVE)
The 4 pending Photo Editor branding fixes are landed on PR #28's branch (`claude/wed-2024-face-match-t4wre2`, "Make the guest Photo Editor real", still an open draft) and deployed live: (1) removed the `מופעל על ידי Oura` line, (2) frame-off now exports a fully clean photo (zero baked branding — the frame toggle is the branding switch), (3) enlarged/emboldened the PHOTO SANTOS studio name+logo to the design's prominence, (4) verified against real exported JPEGs. White-label principle recorded in `CLAUDE.md`. Live `oura-web` `5db18c9a`. Deep-link to see it: open a consented `WED-2024` guest photo in `https://oura-web.oura-events.workers.dev/gallery` → tap the tune (עריכה) icon → toggle "מסגרת Photo Santos" on/off. **PR #28 is still a draft (the founder's own PR); left un-merged pending his ready/merge call — the 4 fixes it now carries are already live regardless.** Other open PRs unchanged: #29 (design-index root-cause docs), #13, plus the stale-diff trio #16/#4/#7 below.
