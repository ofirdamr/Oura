...
**FACE-MATCH — ROOT-CAUSED 2026-07-11 (the recurring "works, then breaks after a day"):** the founder clarified matching works then dies "after a day passes." That is the daily 3am cron (`scheduledCleanup.ts`, `0 3 * * *`). The OLD code `.delete()`d `face_embeddings` for any expired-consent guest, tearing out the SHARED photo index (see MISTAKES 2026-07-09). The code was fixed to `.update({guest_id:null})` (unlink, never delete) — but **that fix never reached production until the 2026-07-11 API redeploy**, because the leading-space `CLOUDFLARE_API_TOKEN` had been silently failing every deploy. So the safe cron is now live for the first time (api `5be81627`); tonight's 3am run should unlink, not delete, and matching should persist.
Diagnostic added this session — `GET /admin/embed-status?event_id=` (bearer `ADMIN_BACKFILL_TOKEN`, currently `oura-backfill-fixed-20260711`). For `WED-2024` (`3c5e60a0-ac85-4682-9be4-427bc04af0a5`) it returned **17 photos / 15 done / 262 face_embeddings / 96 clusters** — so the index is intact right now, NOT wiped. Sandbox can't reach Supabase directly (proxy 502 "policy unavailable"); use the live worker's admin endpoints instead.
Also lowered `GUEST_MATCH_THRESHOLD` 0.42→0.35 (selfie vs event photos was too strict).
**PIPELINE PROVEN HEALTHY 2026-07-11** via new `GET /admin/match-test?event_id=[&photo_id=]` (bearer `ADMIN_BACKFILL_TOKEN`): it re-embeds one of the event's OWN photos and runs match_faces. Result on WED-2024: every face matched its stored self at **distance 0**, same-person faces at **0.54–0.74 similarity**, different people **0.11–0.28**. So embed service + stored vectors + match_faces all work; photo→photo matching is fine at 0.35. **Therefore the founder's failure is SELFIE-SPECIFIC** — either his selfie face isn't detected (→ 422 `no_face_detected`) or his selfie→photo similarity is under 0.35. CANNOT progress without his actual selfie image.
**BLOCKED ON FOUNDER:** need one real selfie from him. Next session: run it through the same embed→match_faces path (add a POST `/admin/selfie-test` that takes an uploaded image, or reuse the guest selfie route with a fresh token) and read the actual distances → then either set the exact threshold or find his face isn't in the index / isn't detected. Do NOT keep lowering the threshold blindly (different-people similarity reaches ~0.28–0.30, so going much below ~0.32 risks cross-matching strangers = privacy problem).
**STILL VERIFY:** after the next 3am cron, confirm `embed-status` still shows ~262 embeddings (proves the delete→unlink cron fix, finally deployed today, holds). **HARDENING TODO:** DB-level guard so `face_embeddings` can never be per-guest-deleted again (migration; must run where Supabase is reachable — sandbox network policy blocks it, 502 "policy unavailable").
