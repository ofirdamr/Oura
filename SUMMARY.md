...
## 🚨 2026-07-14 — Intermittent "face not recognized" = embed-service COLD START, not matching quality
...
**DURABLE FIX STILL NEEDED (founder action — needs GCP access this sandbox lacks):** set Cloud Run `oura-embed` **min-instances = 1** so the container never scales to zero and the model stays warm. The retry reduces the miss frequency; min-instances=1 eliminates the cold start entirely. This is the real cure — do it in the GCP console (Cloud Run → oura-embed → Edit & deploy new revision → Minimum instances = 1).
