...
## 📌 OPEN NOW — Design-fidelity audit + MVP truth-up (2026-07-12), branch `claude/design-fidelity-audit-mvp-4f1mk5`
Four-phase mission (canonical map = `docs/ARCHITECTURE.md` §6b; design is king, code is 1:1 with `design/screens/*/screen.png`, screenshot wins on conflict): **(1) open-PR inventory + decisions [DONE this session], (2) per-screen 1:1 audit, (3) PRD↔flow reconciliation, (4) functional verification.** Handing off between phases at ~30% context.

### Phase 1 — open-PR inventory + decisions (DONE 2026-07-12)
...
- **#28** — *Make the guest Photo Editor real* (`claude/wed-2024-face-match-t4wre2`, draft). **KEEP-WAITING (mistake-parked).** Real feature, but founder rejected the baked branding → **4 fixes still PENDING, not yet coded:** (1) strip `מופעל על ידי Oura` line (`apps/web/app/photo-editor/page.tsx:136` + in `BrandedFrame`); (2) `compositeBrandedPhoto` frame-off must yield a fully clean photo (zero baked branding); (3) enlarge photographer logo to the design's prominence; (4) verify with a REAL exported JPEG vs `design/screens/oura_final_production_photo_editor_desktop/screen.png`, then deploy + live link. `photo_editor` design = white-label (studio brand baked, **no Oura credit**) — the 4 fixes are design-faithful, not new design. **Do the fixes ON this branch** (not a parallel main-based one — avoids the #9/#17/#18 pileup); merge #28 once they land + real-JPEG verified. This is Phase-2 work (Photo Editor screen).
- **#16** — *Cut per-session token baseline* (`claude/token-usage-analysis-ih29wo`, draft, docs/skill only). **KEEP-WAITING (stale, mistake-parked).** Goal (trim the always-loaded baseline) still valid, but the diff is stale: real conflicts in all 5 touched files (`SUMMARY.md`/`PROGRESS.md`/`MISTAKES.md`/`CLAUDE.md`/`universal-framework/SKILL.md`), and it competes with #4 on the SKILL.md trim (different mechanism: sub-skill vs references file). Needs a dedicated docs-trim session to re-derive against current content and pick one SKILL.md approach — not a blind merge. This is the canonical token-trim tracker.
- **#4** — *Trim universal-framework skill* (`claude/framework-skill-evaluation-ijjxtt`, draft, skill only). **KEEP-WAITING (stale).** Moves 4 rare-case protocols out of SKILL.md into `references/escalation-and-handoff.md`. Current SKILL.md still has all 4 inline (unapplied) AND has evolved well past #4's base (fuller roster/§1a), so the diff no longer applies cleanly and its roster hunk would revert improvements. Overlaps #16 on the same file — resolve both together in the docs-trim session; pick #16's or #4's SKILL.md approach, not both.
- **#7** — *Docs: process lessons* — **CLOSED 2026-07-12 (superseded).** Its "wait for founder's explicit merge before touching main" lesson directly contradicts the current `CLAUDE.md` rule "Merge is not an ask-first gate" (reversed 2026-07-10 after the pileup). Other two lessons already covered (stop-when-stuck in the skill; Playwright/proxy blind-spot in SUMMARY's gift-box note). Closing comment on the PR.
...
- **Open-PR state as of 2026-07-12 (Phase 1 done — see the OPEN NOW block at the top for full rationale):** **#28** (Photo Editor real — keep-waiting on 4 branding fixes), **#16** (token-baseline trim — keep-waiting, stale 5-file diff), **#4** (SKILL.md trim — keep-waiting, stale, overlaps #16). **#7 CLOSED** (superseded; its merge-authority lesson contradicts the current "merge is not an ask-first gate" rule).
