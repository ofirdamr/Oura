...
### Phase 2 in progress — per-screen 1:1 audit (2026-07-12, branch `claude/design-fidelity-audit-mvp-4f1mk5-7vm0yl`, draft **PR #32**)
Continues Phase 1 (this branch contains #31's commit `9ee6388` + the fixes below). Audit the §6b "Real" screens one at a time vs their `design/screens/*/screen.png` (screenshot wins), fixing only genuine drift/RTL, no invented features. **Intentionally accumulating on one branch/PR #32 across the phase** (not merge-per-screen) — the code is already deployed live, so the pileup risk is covered by this doc naming it; DON'T rebuild these screens elsewhere.
- **`/gallery-entry`** — audited, verified 1:1, no change. Only deviation is the footer, where the Stitch source itself uses a dead `href="#"` help-center link that the code intentionally replaced with working guidance text (kept — reverting to a dead link would violate "working buttons only").
- **`/admin/create-event`** — fixed 2 genuine RTL inversions (modal header title/close side; action-row wide-primary vs cancel side), both confirmed by `getBoundingClientRect()` measurement on the real built page. **DEPLOYED LIVE** `oura-web` version `9c440013` — see `https://oura-web.oura-events.workers.dev/admin/create-event` (photographer login). Screenshot on local build (authed `/admin/*` is the sandbox live-browser blind spot).
- **Still to audit (§6b Real screens):** `/gallery`, `/gift-reveal` (both have founder-approved deviations from their static Stitch PNGs — audit with care, don't revert the approved premium rebuild), `/consent` (no design PNG — designed fresh, skip 1:1), `/admin`, `/admin/branding`, `/admin/qr-management`, `/admin/events`, `/admin/events/[event_id]`.
