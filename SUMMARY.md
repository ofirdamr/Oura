...
- **Cloud Run deploy STILL BLOCKED — IAM must be granted at SA resource level, not project level** — Tried twice (runs #29869592144, #29870429370). Project-level `Service Account User` for `oura-deploy` does NOT work for Google-managed SAs. Must grant directly ON the specific SA. **Exact fix**: open https://console.cloud.google.com/iam-admin/serviceaccounts/details/932309994000-compute@developer.gserviceaccount.com?project=ouraforphotographers → PERMISSIONS tab → GRANT ACCESS → new principal: `oura-deploy@ouraforphotographers.iam.gserviceaccount.com` → role: Service Account User → Save. Then re-trigger `deploy-cloud-run.yml` on `main` via workflow_dispatch.
