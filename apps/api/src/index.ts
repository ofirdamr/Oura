...
app.post('/auth/forgot-password', async (c) => {
  ...
  console.log('generateLink response:', {
    ...
    actionLinkIsString: typeof actionLink === 'string',
    actionLinkValue: JSON.stringify(actionLink),
  });
  ...
  // Also check that action_link is a non-empty string URL, not null/"null"/undefined.
  if (linkErr || !actionLink || typeof actionLink !== 'string' || actionLink === 'null') {
    console.log('Early return: linkErr or invalid action_link', { linkErr, actionLink });
    return c.json({ ok: true });
  }

  const resetLink = actionLink;
  console.log('resetLink value:', resetLink);
  ...
  console.log('Email body to send:', JSON.stringify(emailBody, null, 2));
  ...
  if (!brevoResponse.ok) {
    ...
    console.log('Brevo error:', brevoError);
  }
